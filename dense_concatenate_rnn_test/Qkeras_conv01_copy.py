...
def qmodels_path(i): return f"tests/qmodels/qmodels_lr={lr}_patiencES={patience_es}_patienceRLR={patience_rlr}_mindelta{delta},rkernel<{bit_lenghts(i)},{integer}>po2:8,rest10.h5"
def qpredicts_path(i): return f"tests/qpredicts/qmodels_lr={lr}_patiencES={patience_es}_patienceRLR={patience_rlr}_mindelta{delta},rkernel<{bit_lenghts(i)},{integer}>po2:8,rest10.npy"

#path of the qconverted models
def qhist_path(i): return f"tests/qhist/qmodels_lr={lr}_patiencES={patience_es}_patienceRLR={patience_rlr}_mindelta{delta},rkernel<{bit_lenghts(i)},{integer}>po2:8,rest10.pkl"
...
dense_bit_width = {'bits':  8, 'integer': integer, 'symmetric': 0, 'alpha':1}
def quantized_conv_model (bits, units_parameter, model_to_convert):  
    ...
    x = qkeras.QDense(1, activation="relu",
                        kernel_quantizer=quantized_po2(),
                        bias_quantizer=quantized_po2())(inputs_past)
