...
def qmodels_path(i,v): return f"tests/qmodels/qmodels_lr={lr}_patiencES={patience_es}_patienceRLR={patience_rlr}_mindelta{delta},homo<{bit_lenghts(i)},{integer}>v{version(v)}.h5"
def qpredicts_path(i,v): return f"tests/qpredicts/qmodels_lr={lr}_patiencES={patience_es}_patienceRLR={patience_rlr}_mindelta{delta},homo<{bit_lenghts(i)},{integer}>v{version(v)}.npy"

#path of the qconverted models
def qhist_path(i,v): return f"tests/qhist/qmodels_lr={lr}_patiencES={patience_es}_patienceRLR={patience_rlr}_mindelta{delta},homo<{bit_lenghts(i)},{integer}>v{version(v)}.pkl"
...
def quantized_conv_model (bits, units_parameter, model_to_convert):  
    ...
    while val_loss > 0.0005:    
        ...
        x = qkeras.QDense(1, activation="relu",
                            ...
                            bias_quantizer=quantized_bits(**bits))(inputs_past)
        ...
        outputs = QDense(output, 
                            activation='relu',
                            kernel_quantizer=quantized_bits(**bits),
                            bias_quantizer=quantized_bits(**bits))(x)
