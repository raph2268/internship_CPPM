...
def models_path(v) : return f"tests/models/models_patiencES={patience_es}_patienceRLR={patience_rlr}_mindelta{delta}_v{version(v)}.h5"      

#path of the tested models 
def hist_path(v) : return f"tests/hist/models_patiencES={patience_es}_patienceRLR={patience_rlr}_mindelta{delta}_v{version(v)}.pkl"     
...
refmodels_path = f"tests/models/models_patiencES=12_patienceRLR=5_mindelta{delta}_v4.h5"      
...
def quantized_conv_model (bits, units_parameter, model_to_convert):  
    ...
    hist = qr_model.fit([X_past, X],y,validation_data=([X_valid_past, X_valid],y_valid),epochs = nbr_conv_epoch, batch_size=nbr_batch, shuffle=True, callbacks=[model_checkpoint_callback])
