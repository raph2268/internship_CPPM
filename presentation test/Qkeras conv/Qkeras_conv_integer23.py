...
def qmodels_path(i,v,k): return f"tests/qmodels/qmodels_conv_lr={lr}_patiencES={patience_es}_patienceRLR={patience_rlr}_mindelta{delta},rkernel<{bit_lenghts(i)},{integer}>_rest={rest_array[k]}_v{version(v)}.h5"

#path of the qconverted models
def qhist_path(i,v,k): return f"tests/qhist/qmodels_conv_lr={lr}_patiencES={patience_es}_patienceRLR={patience_rlr}_mindelta{delta},rkernel<{bit_lenghts(i)},{integer}>_rest={rest_array[k]}_v{version(v)}.pkl"
