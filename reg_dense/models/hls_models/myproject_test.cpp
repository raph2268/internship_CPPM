...
int main(int argc, char **argv)
{
  ...
  std::ifstream fin("/atlas/chiedde/Documents/CPPM_Nemer/cppm-nnlar/hls4ml/quartus-test-cppm/tb_data/data_new.txt");  //oficial
  //std::ifstream fin("/atlas/chiedde/Documents/CPPM_Nemer/cppm-nnlar/hls4ml/quartus-test-cppm/tb_data/data_new_10.txt");   //teste do ressource
  //load predictions from text file
  //std::ifstream fpr("/atlas/chiedde/Documents/CPPM_Nemer/cppm-nnlar/hls4ml/quartus-test-cppm/tb_data/true_lauri.txt");  //oficial lstm
  //std::ifstream fpr("/atlas/chiedde/Documents/CPPM_Nemer/cppm-nnlar/hls4ml/quartus-test-cppm/tb_data/true_lauri_10.txt");  //teste do ressource lstm
  std::ifstream fpr("/aidaq/Nemer_Neural_Network/macro/data/vanilla_8unit_predictions.txt"); //vanilla
  ...
  if (fin.is_open() && fpr.is_open()) {
    ...
    for (; std::getline(fin,iline) && std::getline (fpr,pline); num_iterations++) {
      if (num_iterations % CHECKPOINT == 0) {
        if(num_iterations == 10){ //aqui para o codigo
           break;
        }
      ...
      std::copy(in_begin, in_end, inputs.back().q_dense_429_input);
      in_begin = in_end;
    ...
    for(int i = 0; i < num_iterations; i++) {
      inputs.emplace_back();
      outputs.emplace_back();
      std::fill_n(inputs[i].q_dense_429_input, N_INPUT_1_1, 0.0);
    }
