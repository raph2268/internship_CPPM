...
struct simple_rnn_config: public nnet::activ_config{
...
template<class data_T, class res_T, typename CONFIG_T, class WEIGHT_T>
  void simple_rnn_network(data_T input0[CONFIG_T::n_timestamp], res_T res[CONFIG_T::n_out],
const WEIGHT_T *kernel, const WEIGHT_T *rec_kernel, const WEIGHT_T *bias){
  ...
  */
  ...
  for (int i=0; i < CONFIG_T::n_timestamp; i++){
    ...
    simple_rnn_cell<data_T,CONFIG_T,WEIGHT_T>(hidden_state_temp,h,inputs[i], kernel, rec_kernel, bias);
    #pragma unroll
    for (int x = 0; x < CONFIG_T::n_in; x++) {
...
void simple_rnn_cell(
        ...
        typename simple_rnn_config::fixed_p_internal_t afterW[CONFIG_T::n_in] hls_register;
        multiply_W<data_T,simple_rnn_config::fixed_p_internal_t,CONFIG_T,WEIGHT_T>(inputs, afterW, kernel);
        //Bias addition
        typename simple_rnn_config::fixed_p_internal_t afterBias[CONFIG_T::n_in] hls_register;
        add_bias<simple_rnn_config::fixed_p_internal_t,CONFIG_T,WEIGHT_T>(afterW,afterBias, bias);

        //hidden
        typename simple_rnn_config::fixed_p_internal_t hiddenCand[CONFIG_T::n_in] hls_register;
        multiply_U<data_T,simple_rnn_config::fixed_p_internal_t,CONFIG_T,WEIGHT_T>(hidden_state, hiddenCand, rec_kernel);
        typename simple_rnn_config::fixed_p_internal_t afterAdd[CONFIG_T::n_in];
        add_vectors<simple_rnn_config::fixed_p_internal_t, CONFIG_T>(afterBias, hiddenCand, afterAdd);

        data_T h[CONFIG_T::n_in]/* hls_register*/;
        nnet::relu<simple_rnn_config::fixed_p_internal_t,data_T,CONFIG_T>(afterAdd, h);  //recurrent_activation
       OUTPUT_WRITE_LOOP:
        #pragma unroll
        for (int x = 0; x < CONFIG_T::n_in; x++) {
